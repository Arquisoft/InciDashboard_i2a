...
public class IncidentTest {
	...
	public void inciNameDescriptionTest() {
		assertEquals("Fire", inci1.getName());
		assertEquals("Rain", inci2.getName());
		
		inci1.setName("Fire in university building");
		assertEquals("Fire in university building", inci1.getName());
		
		inci2.setName("Chained car accident");
		assertEquals("Chained car accident", inci2.getName());
		
		assertEquals("Intense fire in the EII at UNIOVI", inci1.getDescription());
		assertEquals("Rain storm affecting A-66", inci2.getDescription());
		
		inci1.setDescription("Intense fire at EII");
		assertEquals("Intense fire at EII", inci1.getDescription());
		
		inci2.setDescription("Rain storm affecting A-66 state");
		assertEquals("Rain storm affecting A-66 state", inci2.getDescription());		
	}
	
	@Test
	public void inciAgentIdTest() {
		assertEquals(agentId[0], inci1.getAgentId());
		assertEquals(agentId[1], inci2.getAgentId());
		
		inci1.setAgentId(agentId[1]);
		assertEquals(agentId[1], inci1.getAgentId());

		inci2.setAgentId(agentId[0]);
		assertEquals(agentId[0], inci2.getAgentId());
	}
	
	@Test
	public void inciAgentKindCodeTest() {
		assertEquals(AgentKind.PERSON, inci1.getAgentKind());
		assertEquals(AgentKind.ENTITY, inci2.getAgentKind());
		
		inci1.setKindCode(2);
		assertEquals(AgentKind.ENTITY, inci1.getAgentKind());
		inci2.setKindCode(3);
		assertEquals(AgentKind.SENSOR, inci2.getAgentKind());
	}
	
	@Test
	public void inciTagsTest() {
		assertEquals(3, inci1.getTags().size());
		assertEquals(3, inci2.getTags().size());
		
		inci1.addTag("tag test 1");
		assertEquals(4, inci1.getTags().size());
		
		inci2.addTag("tag test 2");
		assertEquals(4, inci2.getTags().size());
	}
	
	@Test
	public void inciLocationTest() {
		assertEquals(43.354929, inci1.getLocation().getLat(), 0.0001);
		assertEquals(-5.851416, inci1.getLocation().getLng(), 0.0001);
		
		assertEquals(43.3584404, inci2.getLocation().getLat(), 0.0001);
		assertEquals(-5.8284433, inci2.getLocation().getLng(), 0.0001);
		...
		assertEquals(43.3584404, inci1.getLocation().getLat(), 0.0001);
		assertEquals(-5.8284433, inci1.getLocation().getLng(), 0.0001);
		
		inci2.getLocation().setLat(43.354929);
		inci2.getLocation().setLng(-5.851416);
		assertEquals(43.354929, inci2.getLocation().getLat(), 0.0001);
		assertEquals(-5.851416, inci2.getLocation().getLng(), 0.0001);
	}
	
	@Test
	public void inciStateTest() {
		assertEquals(InciState.OPEN, inci1.getState());
		assertEquals(InciState.OPEN, inci2.getState());
		
		inci1.setState(InciState.INPROCESS);
		assertEquals(InciState.INPROCESS, inci1.getState());
		
		inci2.setState(InciState.INPROCESS);
		assertEquals(InciState.INPROCESS, inci2.getState());
	}
	
	@Test
	public void inciMultimediaTest() {
		assertEquals(2, inci1.getMultimedia().size());
		assertEquals(2, inci2.getMultimedia().size());
		
		inci1.addFile("intervention1.txt");
		assertEquals(3, inci1.getMultimedia().size());
		
		inci2.addFile("intervention2.txt");
		assertEquals(3, inci2.getMultimedia().size());
		inci1.setMultimedia(new ArrayList<String>());
		assertTrue(inci1.getMultimedia().isEmpty());
		
	}
	
	@Test
	public void inciPropertiesTest() {
		assertEquals(3, inci1.getProperties().size());
		assertEquals(3, inci2.getProperties().size());
		
		inci1.getProperties().put("intervention", true);
		inci2.getProperties().put("intervention", true);
		
		assertEquals(4, inci1.getProperties().size());
		assertEquals(4, inci2.getProperties().size());
		inci1.setProperties(new HashMap<String,Object>());
		assertTrue(inci1.getProperties().size()==0);
	}
	
	@Test
	public void inciOperatorTest() {
		assertEquals(op1, inci1.getOperator());
		assertEquals(op2, inci2.getOperator());
		
		inci1.setOperator(op2);
		inci2.setOperator(op1);
		assertEquals(op2, inci1.getOperator());
		assertEquals(op1, inci2.getOperator());
		inci1.setTags(new ArrayList<String>());
	}
	
	@Test
	public void inciCommentsTest() {
		assertEquals(2, inci1.getComments().size());
		assertEquals(2, inci2.getComments().size());
		
		inci1.addComment("Incident managed successfully");
		inci2.addComment("Incident managed successfully");
		
		assertEquals(3, inci1.getComments().size());
		assertEquals(3, inci2.getComments().size());
		
		inci1.setComments(new ArrayList<String>());
		assertTrue(inci1.getComments().isEmpty());
		
		Incident i2 = new Incident();
		i2.setId(new ObjectId("507f1f77bcf86cd799439011"));
		assertEquals("507f1f77bcf86cd799439011", i2.getId().toString());
		i2.setKindCode(2);
		assertEquals(i2.getKindCode(),2);
		assertEquals(AgentKind.values()[1],i2.getAgentKind());
	}
	...
	public void testCreator() {
		...
		assertEquals("intervention=\"true\"", print.toString());
